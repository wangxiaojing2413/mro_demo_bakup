    ...
    controller('tvTreeController', ['$scope', '$rootScope', '$element', function($scope, $rootScope, $element) {
        ...
        $scope.ApplyNodeSelectionList = function() {
            ...
            if ($scope.currentModelWidget) {
                for (var i=0; i<$scope.nodeSelection.length; i++) {
                    $scope.currentModelWidget.SelectPart($scope.nodeSelection[i], true, Module.SelectType.SELECT);
                }
            }
        }
        ...
        $scope.AddInstances = function(messages) {
            for (var i = 0; i < messages.length; i++) {
                ...
                $scope.SetCurrentModelWidget(modelId);
                if ($scope.currentModelWidget) {
                    //var name = $scope.currentModelWidget.GetInstanceName(strippedIdpath)
                    var fullIdpath = PrependModelId(idpath, $scope.currentModelWidget.id);
                    if (idpath == ':') {
                        // Create new tree
                        if (!$scope.tvTree) {
                    ...
                    if (event.ctrlKey) {
                        ...
                        if (i != -1) {
                            ...
                            if ($scope.currentModelWidget) {
                                ...
                                $scope.currentModelWidget.SelectPart(strippedIdpath, false, Module.SelectType.SELECT);
                            }
                        } else {
                            ...
                            if ($scope.currentModelWidget) {
                                var strippedIdpath = $scope.StripModelIdFromIdPath(node.data.idpath);
                                $scope.currentModelWidget.SelectPart(strippedIdpath, true, Module.SelectType.SELECT);
                            }
                        }
                    ...
                    if (event.ctrlKey) {
                        ...
                        for (var i = startUid; i <= endUid; i++) {
                            ...
                            if (selNode != undefined) {
                                ...
                                $scope.SetCurrentModelWidget(GetModelId(selNode.data.idpath));
                                if ($scope.currentModelWidget) {
                                    $scope.currentModelWidget.SelectPart(strippedIdpath, true, Module.SelectType.SELECT);
                                }
                            }
                ...
                if ($scope.currentModelWidget) {
                    if (event.ctrlKey) {
                        ...
                        if (n != -1) {
                            ...
                            $scope.currentModelWidget.SelectFeature(idPath, id, !$scope.CheckModelAnnotationSelected(idPath, id));
                        }
                    } else if (event.shiftKey) {
                        // Do nothing yet
                    } else {
                        ...
                        if (n != -1) {
                            ...
                            $scope.currentModelWidget.SetFeatureSelection(idPath, idPathArr);
                        }
                    }
                }
            }
                ...
                for (var i=0; i<keys.length; i++) {
                    ...
                    if (features) {
                        for (var j=0; j<features.length; j++) {
                            ++$scope.ignoreFeaturePopulate;
                            $scope.currentModelWidget.SelectFeature(keys[i], features[j], false);
                        }
                    }
                }
            } else {
